 ...
 *           "prefix" → transfer_2_crm_to_analytics
 *           "suffix" → transfer_crm_to_analytics_2
 *           "random" → transfer_crm_to_analytics-a7b2 (legacy)
...
const DB_DB_PARENT_PATH = "";  // Set this to parent path if needed, e.g., "Documentation/Data Flow/"
...
// "prefix" → transfer_2_crm_to_analytics, transfer_3_crm_to_analytics (number after "transfer")
// "suffix" → transfer_crm_to_analytics_2, transfer_crm_to_analytics_3 (number at end)
// "random" → transfer_crm_to_analytics-a7b2 (random 4-char suffix - legacy behavior)
...
async function createTransferNote(objectAPath, objectBPath, classification, isBidirectional = false, suffix = "") {
  ...
  if (TRANSFER_NUMBERING_MODE === "random") {
    ...
    if (TRANSFER_NUMBERING_MODE === "prefix") {
      // transfer_2_crm_to_analytics, transfer_3_crm_to_analytics
      if (nextNum === 1) {
      ...
      // transfer_crm_to_analytics_2, transfer_crm_to_analytics_3
      if (nextNum === 1) {
        // First file - no number
        fileName = `transfer${OBJECT_SEPARATOR}${objA}${OBJECT_SEPARATOR}${direction}${OBJECT_SEPARATOR}${objB}${suffix}`;
      } else {
...
(async () => {
  ...
  clog("\n✅ Linkify DFD v1.5: finished");
  note("Linkify DFD v1.5: finished");
